...
- **⚡ Responsive UI lane** — bar buttons and inline callbacks now also ride a dedicated `control:<chat>` outbound queue, so Goal/Todos/Queue/Collapse react instantly while assistant streaming occupies the content queue
...
- **🗂️ Project-grouped sessions** — the Sessions card mirrors the web display-title chain (`session/title` → cwd basename → id), groups sessions by workspace project, opens the running project first, offers a `🔀 Projects` switcher, per-row `Archive`/`Delete` actions, and hides archived sessions with a live `🗄N` count
- **💡 Bar control** — Menu page 1 has a `💡 Collapse Bar / Show Bar` switch and `/bar [on|off]` toggles the keyboard; the bar's `🗜️ Collapse` hides it without leaving any carrier message behind
...
- **🗂️ Workspaces are usable** — a Workspace detail card has `✅ Use this project` (set active project for new sessions) and `🧭 Sessions` (open its sessions)
